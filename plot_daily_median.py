...
OUTPUT_FILE = "daily_median_time_series.png"
MIN_RATIO = 0.9
...
for filepath in glob.glob(FILES_PATTERN):
    ...
    # convert median columns
    for v in VARS:
        df[f"median_{v}"] = pd.to_numeric(df.get(f"median_{v}"), errors="coerce")
...
# determine global date range from filtered data
all_dates = []
...
for ax, var in zip(axes, VARS):
    for sensor, df in sensor_dfs.items():
        ax.plot(df['date'], df[f'median_{var}'], label=sensor, color=color_map[sensor])
...
fig.suptitle(f"Daily median from {start_date} to {end_date}", fontsize=16)
